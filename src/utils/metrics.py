## WIP central utility for logging to tensorboard or something else
...
def init_metrics(trainer_instance):
    global writer
    writer = train_utils.get_writer(trainer_instance)


def log_scalar(tag, value, global_step, skip_zero=True):
    global writer
    if writer is None:
        print(f"Writer not initialized, cannot log image for tag {tag} at step {global_step}")
        return
...
def log_tensor_stats(tag, tensor, global_step):
    global writer
    if writer is None:
        print(f"Writer not initialized, cannot log image for tag {tag} at step {global_step}")
        return
    ...
    if isinstance(tensor, torch.Tensor):
        writer.add_scalar(f"{tag}/mean", tensor.mean().item(), global_step)
        writer.add_scalar(f"{tag}/std", tensor.std().item(), global_step)
        writer.add_scalar(f"{tag}/min", tensor.min().item(), global_step)
        writer.add_scalar(f"{tag}/max", tensor.max().item(), global_step)
        writer.add_scalar(f"{tag}/norm", tensor.norm().item(), global_step)
        writer.add_scalar(f"{tag}/any_nan", tensor.isnan().any().item(), global_step)
        writer.add_scalar(f"{tag}/any_inf", tensor.isinf().any().item(), global_step)

def log_image(tag, image_tensor, global_step):
    global writer
    if writer is None:
        print(f"Writer not initialized, cannot log image for tag {tag} at step {global_step}")
        return
    ...
    if isinstance(image_tensor, torch.Tensor):
        ...
        if img_max > img_min:  # Avoid division by zero
            normalized_image = (image_tensor - img_min) / (img_max - img_min)
        else:
            normalized_image = torch.zeros_like(image_tensor)
        writer.add_image(tag, normalized_image, global_step)

def log_vocoder_audio(tag: str, vocoder, mel_spec: torch.Tensor, global_step: int, audio_sample_rate: int = 16000):
    """Convert mel spectrogram to audio using vocoder and log to TensorBoard."""

    global writer
    if writer is None:
        print(f"Writer not initialized, cannot log audio for tag {tag} at step {global_step}")
        return
...
def visualize_mel_spec(mel_spec: np.ndarray, audio_hop_lenght=256, audio_sample_rate=16000, audio_n_fft=1024) -> np.ndarray:
...
def log_mel_comparison(tag: str, pred_mel: np.ndarray, target_mel: np.ndarray, global_step: int):
...
def log_attention_weights(
    ...
    Args:
        ...
        tag: Tag prefix for TensorBoard (e.g., "eval_vae/example_0/encoder_attention")
    ...
    plt.tight_layout()
    writer.add_figure(f"{tag}/global_2d", fig, global_step)
    plt.close(fig)
    ...
    for head_idx, ax in enumerate(axes):
        ...
        ax.set_ylabel('Query')
    plt.tight_layout()
    writer.add_figure(f"{tag}/per_head", fig, global_step)
    plt.close(fig)
    ...
    plt.tight_layout()
    writer.add_figure(f"{tag}/spatial_pattern", fig, global_step)
    plt.close(fig)