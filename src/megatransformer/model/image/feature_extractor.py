...
from megatransformer.config.image.feature_extractor import IMAGE_PRELUDE_CONFIGS, ImageVAEPreludeFeatureExtractorConfig
from megatransformer.model.image.vae.vae import ImageVAEEncoder
from megatransformer.model.norms import create_norm
from megatransformer.model.transformer import MegaTransformerAxial2DEncoderBlock, MegaTransformerEncoderBlock
...
class ImageVAEPreludeFeatureExtractor(nn.Module):
    ...
    def from_config(cls, config_name: str, vae_encoder: Optional[ImageVAEEncoder]=None, **overrides) -> "ImageVAEPreludeFeatureExtractor":
        ...
        """
        if config_name not in IMAGE_PRELUDE_CONFIGS:
            raise ValueError(f"Unknown config: {config_name}. Available: {list(IMAGE_PRELUDE_CONFIGS.keys())}")

        config = IMAGE_PRELUDE_CONFIGS[config_name]
        # Apply overrides
        config_dict = {k: v for k, v in config.__dict__.items()}
        config_dict.update(overrides)
        config = ImageVAEPreludeFeatureExtractorConfig(**config_dict)
