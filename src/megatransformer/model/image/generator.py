...
from megatransformer.config.image.generator import IMAGE_CODA_CONFIGS, ImageCodaAndVAEConfig
...
class ImageCodaAndVAEWithLoss(nn.Module):
    ...
    @classmethod
    def from_config(cls, config_name: str, vae_decoder: Optional[ImageVAEDecoder]=None, **overrides) -> "ImageCodaAndVAEWithLoss":
        """
        ...
        Example:
            model = ImageCodaAndVAEWithLoss.from_config("small", vae_decoder=my_vae_decoder, coda_config=custom_coda_config)
        """
        if config_name not in IMAGE_CODA_CONFIGS:
            raise ValueError(f"Unknown config: {config_name}. Available: {list(IMAGE_CODA_CONFIGS.keys())}")

        config = IMAGE_CODA_CONFIGS[config_name]
        # Apply overrides
        config_dict = {k: v for k, v in config.__dict__.items()}
        config_dict.update(overrides)
        config = ImageCodaAndVAEConfig(**config_dict)
