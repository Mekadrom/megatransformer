...
from megatransformer.config.voice.vocoder.vocoder import VOCODER_CONFIGS, VocoderConfig
from megatransformer.model import activations
from megatransformer.model.voice.vocoder.convnext import ConvNeXtBlock
from megatransformer.model.voice.vocoder.frequency_attention_block import FrequencyAttentionBlock
...
class Vocoder(nn.Module):
    ...
    def forward(
        ...
        pred_stft = torch.complex(stft_real.to(torch.float32), stft_imag.to(torch.float32))[..., :mel_specs.shape[-1]]
        ...
        return {
            ...
            "pred_stft": pred_stft,
            "pred_magnitude": mag,
            "pred_phase_real": phase_real,
            "pred_phase_imag": phase_imag,
        }
