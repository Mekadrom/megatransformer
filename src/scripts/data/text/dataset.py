...
class TextShardedDataset(Dataset):
    """
    Efficient dataset for loading preprocessed text shards.

    Loads shards containing:
    - token_ids: [B, T] tokenized text as integer IDs
    - text_lengths: [B] original token lengths before padding
    - text: list[str] raw text strings
    """
    ...
    def __init__(
        ...
        self.vocab_size = self.config.get("vocab_size", 32007)
    ...
    def _build_and_cache_index(self, index_path: str):
        ...
        print(f"Indexing {len(self.shard_files)} text shards (first time only, will be cached)...")
