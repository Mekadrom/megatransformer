...
Usage:
    ...
    PYTHONPATH=src python3 -m scripts.debug.plot_iteration_stats --checkpoint ./runs/world/<run>/checkpoint-<step> --config huginn_small_causal_cross_attn --output ./stats.png

    # Voice synthesis:
    PYTHONPATH=src python3 -m scripts.debug.plot_iteration_stats --checkpoint ./runs/world/<run>/checkpoint-<step> --modality voice --output ./stats_voice.png

    # Text continuation:
    PYTHONPATH=src python3 -m scripts.debug.plot_iteration_stats --checkpoint ./runs/world/<run>/checkpoint-<step> --modality text --output ./stats_text.png

    # Voice transcription:
    PYTHONPATH=src python3 -m scripts.debug.plot_iteration_stats --checkpoint ./runs/world/<run>/checkpoint-<step> --modality voice --direction transcription --output ./stats_voice_transcription.png

    # Multiple checkpoints (overlaid comparison):
    PYTHONPATH=src python3 -m scripts.debug.plot_iteration_stats --checkpoint ./runs/world/<run>/checkpoint-1000 ./runs/world/<run>/checkpoint-5000 --output ./comparison.png

    # Per-token individual plots:
    PYTHONPATH=src python3 -m scripts.debug.plot_iteration_stats --checkpoint ./runs/world/<run>/checkpoint-<step> --output ./stats.png --per_token
"""
...
def plot_single(iteration_stats, title, output_path):
    ...
    from utils.visualization import render_iteration_stats
...
def plot_per_token(iteration_stats, output_dir, batch_idx=0, modality_map=None):
    ...
    from utils.visualization import render_token_iteration_stats
...
def run_single_task(model, ds, tasks, modality, direction, output_base, per_token=False):
    ...
    from utils.visualization import render_all_tokens_iteration_stats
