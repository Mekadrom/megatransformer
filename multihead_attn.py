...
class MultiHeadAttention(nn.Module):
    def __init__(self, device, model_config, attn_config, self_attn, in_decoder=False):
        ...
        if self.use_infinite_attention:
            ...
            self.register_buffer('causal_mask', torch.tril(torch.ones(self.maxlen + 1, self.maxlen + 1).to(self.device)))
    ...
    def forward(self, query_sequences: torch.Tensor, key_sequences: torch.Tensor, value_sequences: torch.Tensor, key_padding_mask: Optional[torch.Tensor] = None) -> tuple[torch.Tensor, list[torch.Tensor]]:
        ...
        if self.use_infinite_attention:
            ...
            for idx in range(self.infinite_attention_n_segments):
                ...
                attention_weights_for_visualization.append(attention_weights.clone().detach().contiguous().view(N, self.n_gqa_groups, self.n_heads, t // self.infinite_attention_n_segments, T // self.infinite_attention_n_segments))
