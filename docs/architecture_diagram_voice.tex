...
\definecolor{cvaeorange}{HTML}{D35400}
...
% ════════════════════════════════════════════════════════════════════
% CVAE DECODER (below voice output — synthesis decode path)
% ════════════════════════════════════════════════════════════════════

\pgfmathsetmacro\cvaeBaseY{\outY-2}

% 1D stage
\node[modbox=cvaeorange, minimum width=3.4cm] (cvae_1d) at (\cvaeX, \cvaeBaseY+3.2)
  {Conv1d Stage\\+ FiLM (speaker)\\+ Residual Blocks};

% 1D→2D transition
\node[modbox=cvaeorange, minimum width=3.4cm] (cvae_reshape) at (\cvaeX, \cvaeBaseY+5.0)
  {Reshape 1D $\to$ 2D\\$+ f_0$ injection (additive)};

% F0 predictor
\node[modbox=cvaeorange, minimum width=3.0cm] (f0_pred) at ([xshift=-2.5cm]cvae_reshape.west)
...
% 2D stages
\node[modbox=cvaeorange, minimum width=3.6cm] (cvae_2d) at (\cvaeX, \cvaeBaseY+6.8)
  {Conv2d Upsample $\times 3$\\FiLM (speaker) per stage\\$(2\times, 2\times) \to (2\times, 2\times) \to (2\times, 1\times)$};

% Final conv
\node[modbox=cvaeorange] (cvae_out_conv) at (\cvaeX, \cvaeBaseY+8.3)
  {Conv2d $\to$ 1 channel};

% Mel output
\node[iolabel] (mel_out) at (\cvaeX, \cvaeBaseY+9.5)
  {Mel Spectrogram\\$(B, 80, T)$};

% CVAE arrows
\draw[arrow] (voice_proj_out.north) -- (cvae_1d.south);

\draw[arrow] (spk_in) -- (f0_pred);
\draw[arrow] (f0_pred) |- (cvae_1d);
\draw[arrow] (cvae_1d) -- (cvae_reshape);
\draw[arrow] (cvae_reshape) -- (cvae_2d);
\draw[arrow] (cvae_2d) -- (cvae_out_conv);
\draw[arrow] (cvae_out_conv) -- (mel_out);

% Speaker embedding arrows to FiLM layers
\draw[dasharrow, rounded corners=4pt]
  (spk_in.south) |- (cvae_1d.west)
  node[pos=0.2, left, font=\tiny\sffamily\itshape, text=spkpink] {FiLM};
\draw[dasharrow, rounded corners=4pt]
  (spk_in.north) |- (cvae_2d.west)
  node[pos=0.25, left, font=\tiny\sffamily\itshape, text=spkpink] {FiLM};

% F0 to reshape
\draw[dasharrow, rounded corners=4pt]
  (f0_pred.east) -- (cvae_reshape.west)
  node[pos=0.5, yshift=-0.25cm, font=\tiny\sffamily\itshape, text=cvaeorange] {$f_0$ embed};

% CVAE background
\begin{scope}[on background layer]
  \node[groupbg=cvaeorange, inner sep=12pt,
    fit=(spk_in)(f0_pred)(cvae_1d)(cvae_reshape)(cvae_2d)(cvae_out_conv)(mel_out)] (cvaebg) {};
\end{scope}
\node[seclabel=cvaeorange, anchor=north west] at ([xshift=4pt, yshift=-2pt]cvaebg.north west)
  {CVAE Decoder (pretrained, speaker-conditioned)};

% ════════════════════════════════════════════════════════════════════
% VOCODER (final stage)
% ════════════════════════════════════════════════════════════════════

\node[modbox=darkgray, minimum width=3.2cm] (vocoder) at (\cvaeX, \cvaeBaseY+11.0)
  {HiFi-GAN Vocoder\\mel $\to$ waveform};
\node[iolabel] (wav_out) at (\cvaeX, \cvaeBaseY+12.3) {Waveform $(B, S)$};
