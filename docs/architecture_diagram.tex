...
\node[iolabel] (image_in) at (\imageX, 0) {VAE Latents\\$(B, C, H, W)$};
...
\node[attnlabel=voicegreen, anchor=east] at ([xshift=-5pt]voice_prelude.west) {CAUSAL};
...
\node[attnlabel=textblue, anchor=west] at ([xshift=5pt]text_prelude.east) {CAUSAL};
...
\node[attnlabel=imagered, anchor=west] at ([xshift=5pt]image_prelude.east) {BIDIR};
...
\node[modbox=voicegreen, minimum width=2.6cm] (voice_textcond) at (\voiceCondX, 8.0) {Text Cond.\\Cross-Attn};
\node[attnlabel=voicegreen, anchor=east] at ([xshift=-4pt]voice_textcond.west) {BIDIR};
...
\node[modbox=imagered, minimum width=2.6cm] (image_textcond) at (\imageCondX, 8.0) {Text Cond.\\Cross-Attn};
\node[attnlabel=imagered, anchor=west] at ([xshift=4pt]image_textcond.east) {BIDIR};
...
\draw[dasharrow] (text_prelude) -- ++(0, 1.5) -| (voice_textcond)
  node[pos=0.15, above, font=\tiny\sffamily\itshape, text=darkgray] {text hidden states};
\draw[dasharrow] (text_prelude) -- ++(0, 1.5) -| (image_textcond);
...
\draw[arrow] (voice_prelude) -- ++(0, 2.0) -| ([xshift=-4.5cm]interleaver.south);
\draw[arrow] (voice_textcond) -- ++(0, 0.8) -| ([xshift=-3cm]interleaver.south);
\draw[arrow] (text_prelude) -- ++(0, 1.5) -| ([xshift=0cm]interleaver.south);
\draw[arrow] (image_prelude) -- ++(0, 2.0) -| ([xshift=4.5cm]interleaver.south);
\draw[arrow] (image_textcond) -- ++(0, 0.8) -| ([xshift=3cm]interleaver.south);
...
\node[attnlabel=recurrentpurple, anchor=west] at ([xshift=5pt]recblocks.east) {CAUSAL};
...
\begin{scope}[on background layer]
  \node[groupbg=recurrentpurple, inner sep=16pt,
    fit=(scale)(postnorm)(rec_right_pad)(rec_left_pad)] (recbg) {};
\end{scope}
...
\node[attnlabel=voicegreen, anchor=east] at ([xshift=-5pt]voice_coda.west) {CAUSAL};
...
\node[attnlabel=textblue, anchor=west] at ([xshift=5pt]text_coda.east) {CAUSAL};

% Image coda (right, matching input column — cross-attention architecture)
\node[modbox=imagered] (image_norm) at (\imageX, 22.8) {LayerNorm};
...
\node[iolabel, above=0.6cm of voice_proj_out] (voice_out) {Voice Latents $(B, C, T)$};
